...
# Cải tiến bởi Grok 3 - xAI, ngày 28/03/2025
...
sudo dnf install -y epel-release nano telnet bind-utils tar wget sed s-nail
...
# Giữ file chính và thêm !include
...
# Cấu hình mail location và namespace
...
# Kiểm tra cấu hình Dovecot
sudo dovecot -n
check_error "Cấu hình Dovecot có lỗi cú pháp"
...
sudo mkdir -p /home/"$EMAIL_USER"/Maildir/{new,cur,tmp}
...
sudo firewall-cmd --add-service={http,https,smtp,imap} --permanent
sudo firewall-cmd --add-port={143/tcp,993/tcp} --permanent
sudo firewall-cmd --reload
check_error "Cấu hình firewall thất bại"

# Gửi mail thử
echo "Gửi mail thử..."
echo "Test mail content" | s-nail -s "Test Subject" "$EMAIL_USER@$DOMAIN"

# Hoàn tất
echo "Cài đặt hoàn tất!"
echo "Truy cập Roundcube tại: https://$DOMAIN/roundcube"
echo "Đăng nhập với: $EMAIL_USER@$DOMAIN / $EMAIL_PASS"
echo "Kiểm tra log nếu có lỗi: journalctl -u dovecot, journalctl -u postfix, /var/www/html/roundcube/logs/errors.log"
